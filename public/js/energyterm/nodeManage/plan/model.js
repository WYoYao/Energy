v.pushComponent({
  ...
  methods: {
    ...
    postionTree: function() {
      ...
      _that.$nextTick(function() {
        _that.suboptiontop = {
          top:
            $(".plan_content .title").height() +
            $(".plan_content .nodes").height() +
            76 +
            "px"
        };
      });
  ...
  computed: {
    ...
    PlanNodeEditTreeArray: function() {
      var _that = this;
      // 空处理
      if (!_that.PlanNodeEditTree.length)
        return {
          childs: []
        };
      ...
      _that.PlanNodeEditTree = _that.PlanNodeEditTree.map(function(item) {
        item.isplanNode =
          selectedArr.indexOf(item.energyItemId) != -1 ? true : false;

        return item;
      });

      _that.arr2tree(
        _that.PlanNodeEditTree,
        "energyItemId",
        "parentId",
        "childs"
      );
      ...
      try {
        ...
        var obj =
          _that.PlanNodeEditTree.filter(function(item) {
            return (
              item.energyItemId == _that.NodeManageModel.nodes.budget[0].id
            );
          })[0] || {};
