v.pushComponent({
    ...
    methods: {
        ...
        autoAllocate: function () {//点击自动分配按钮
        ...
        saveAllocatePlan: function () {//保存计划
            if (this.allocatePlanType == 1) {//按分项
            ...
            planManController.saveEnergyPlan(this.monthPlanInfo, this.allocatePlanType);
        },
        ...
        planEnergyUpDown: function (item, param) {//上下箭头 click
            ...
            if (param == 'up') {
                item.energyPlanRatio = Math.round((item.energyPlanRatio + 0.1) * 10) / 10;//比例
            } else {
        ...
        planEnergyChange: function (item, event) {//input 输入计划值
        ...
        showCountMachine: function (item, event, index) {//点开计算器
            ...
            //开着 并且点击的是自己
            if (v.instance.countMachineShow && ((this.allocatePlanType == 1 && this.countItemPlan.planItemId == item.planItemId) || (this.allocatePlanType == 0 && this.countItemPlan.time == item.time))) {
        ...
        sureCountResult: function () {//确定计算结果
            ...
            var val = $input.val();
            if (this.planCountPlus) {
                this.countItemPlan.energyDataPlan = Number(this.countItemPlan.energyDataPlan) + Number(val);
            } else {
                this.countItemPlan.energyDataPlan = Number(this.countItemPlan.energyDataPlan) - Number(val);
    ...
    watch: {
        monthPlanInfo: {
            handler: function (newData, oldData) {//要不要挪位置
                if (this.firstPlanInfo) {//如果是获取接口数据  就不需要计算了
                    this.firstPlanInfo = false;
                    return;
                }
                var totalData = 0;
                if ((this.allocatePlanType == 1 && !newData.itemPlan) || (this.allocatePlanType == 0 && !newData.dayPlan)) return;
                if (this.allocatePlanType == 1) {
                    newData.itemPlan.forEach(function (ele) {
                        totalData += testNumber(ele.energyDataPlan) ? Number(ele.energyDataPlan) : 0;
                    });
                } else {
