...
planManController.getEnergyPlanInfo = function (bitem, autoType) { //创建或 编辑计划 
    if (autoType == 'wai') {
        $("#globaloadng").pshow();
    } else {
    ...
    if (ifHasPlan && autoType != 'auto') {//有计划  编辑
    ...
    if (pindex == 1) {//按分项
        ...
        var timeFrom = new Date(planDate.getFullYear(), planDate.getMonth(), planDate.getDate() - firstDay - 7).format("yyyy-MM-dd hh:mm:ss");
        var timeTo = new Date(planDate.getFullYear(), planDate.getMonth() + 1, 1).format("yyyy-MM-dd hh:mm:ss");
    }

    pajax.post({
        url: url,
        data: {
            "buildingId": v.instance.planBackPage != 'budget_page' ? v.instance.currentBuild.buildingId : v.instance.budgetBuildSel.buildingId,                //类型：String  必有字段  备注： 建筑id
        ...
        success: function (res) {
            ...
            var nowday = new Date();
            var yesterday = new Date(nowday.getFullYear(), nowday.getMonth(), nowday.getDate() - 1);//昨天
            var planDate = new Date(bitem.planDate.replace(/-/g, '/'));
            var lastDate = new Date(planDate.getFullYear(), planDate.getMonth() + 1, 0);//计划月的最后一天
            ...
            if (bitem.isNowMonth) {//如果本月
                var dateObj = {
                    pdateStr: planDate.getFullYear() + '年' + (planDate.getMonth() + 1) + '月',
                    monthStr: planDate.format('MM.dd') + '~' + lastDate.format('MM.dd'),
                    passMonthStr: nowday.getDate() > 1 ? planDate.format('MM.dd') + '~' + yesterday.format('MM.dd') : '',
                ...
                var dateObj = {
                    ...
                    monthStr: planDate.format('MM.dd') + '~' + lastDate.format('MM.dd'),
                    passMonthStr: '',
                    afterMonthStr: planDate.format('MM.dd') + '~' + lastDate.format('MM.dd'),
                }
            }
            ...
            if (pindex == 1) {//按分项
                var totalData = 0;
                resData.itemPlan.forEach(function (ele) {//计算比例            
                    ele.energyOccurredRatio = ele.energyOccurredRatio && Math.round(ele.energyOccurredRatio * 10) / 10;
                    var percent = resData.monthRemainData > 0 ? ele.energyDataPlan / resData.monthRemainData : 0;
                    ele.energyPlanRatio = Math.round(percent * 1000) / 10;
                    totalData += ele.energyDataPlan;
                    ele.energyDataPlan = ele.energyDataPlan && Math.round(ele.energyDataPlan);
                    ele.energyDataReal = ele.energyDataReal && Math.round(ele.energyDataReal);
                });

            } else {//按日期
                var totalData = 0;
                resData.dayPlan.forEach(function (ele) {//样式属性
                    ele.isLastMonth = new Date(ele.time.replace(/-/g, '/')).getTime() < new Date(bitem.planDate.replace(/-/g, '/')).getTime() ? true : false;//是不是上个月 
                    ele.isThisMonth = !ele.isLastMonth;
                    ele.isPassDay = new Date(ele.time.replace(/-/g, '/')).getTime() <= yesterday.getTime() ? true : false;//是不是过去的日子
...
planManController.saveEnergyPlan = function (pinfo, pindex) { //保存计划
    $("#globaloadng").pshow();
    var planMap = {};
    if (pindex == 1) {//按分项
    ...
    var params = {
        ...
        "buildingId": v.instance.planBackPage != 'budget_page' ? v.instance.currentBuild.buildingId : v.instance.budgetBuildSel.buildingId,
    ...
    pajax.update({
        ...
        success: function (res) {
            ...
            if (v.instance.planBackPage == 'budget_page') {//从预算进来
...
planManController.getPlanTotalChart = function (pinfo, pindex) { //获取日总计划chart  分项总计划chart
    var chartArr = [];
    if (pindex == 1) {//按分项
        planManController.planItemChart && planManController.planItemChart.destroy();
        var planArr = pinfo.itemPlan;
        ...
        try {
            planManController.planDayChart && planManController.planDayChart.destroy();
        } catch (error) {
        ...
        planManController.planDayChart = columnComparisonChart('itemDayChart', 'day');
...
planManController.generateDayPlanByItemSum = function (planDate, pitem) { //
    pajax.post({
        ...
        data: {
            "buildingId": v.instance.budgetBuildSel.buildingId,                //类型：String  必有字段  备注：无
        ...
        success: function (res) {
            ...
            planArr.forEach(function (ele) {
                var nowday = new Date();
                var yesterday = new Date(nowday.getFullYear(), nowday.getMonth(), nowday.getDate() - 1);//昨天
                var isPassDay = new Date(ele.time.replace(/-/g, '/')).getTime() <= yesterday.getTime();//过去
                var dobj = {
                    x: new Date(ele.time.replace(/-/g, '/')).getTime(),
                    y: isPassDay ? Math.round(ele.realData) : Math.round(ele.planData),
