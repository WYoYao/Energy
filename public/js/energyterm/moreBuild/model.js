...
v.pushComponent({
    ...
    methods: {
        ...
        downLoadDayPDF: function () {
            ...
            argu.timeFrom = new Date($("#daterightdate").psel().startTime).format('yyyy-MM-dd hh:mm:ss');
        ...
        DTimeSel: function () {
            this.Dactivate({
                timeFrom: (new Date($("#daterightdate").psel().startTime)).format("yyyy-M-d h:m:s")
            })
        },
        ...
        handler_build_select: function (item) {
            ...
            if (item.buildingId == 'all') {
                ...
                _that.queryBuildingEnergy(new Date(value.startTime).format("yyyy-M-d h:m:s"), new Date(value.endTime).format("yyyy-M-d h:m:s"))
                // 初始化全部列表
        ...
        handler_date_select: function (item) {
            ...
            canNotSelectFutureDay(+new Date(start), "divCalendar", true);

            if (+new Date() < +new Date(start)) {

                $("#globalnotice").pshow({ text: "请选择当前及历史月份", state: "failure" });

                var date = new Date(Datehistorys.slice(-1));
        ...
        queryBuildingEnergy: function () {
            ...
            // 查询当前项目下面的能耗
            return more_build_controller.FNPJ_GetBuildingEnergyOutline(argu).then(function (res) {
        ...
        queryBuildingsOfProject: function (timeFrom, timeTo) {
            ...
            // 查询所有建筑的能耗信息F
            return more_build_controller.FNPJ_GetBuildingsOfProject().then(function (res) {
        ...
        saveAlarm: function () {
            ...
            var _that = this, model = _that.AlarmAndWarning[0], argu;
            ...
            more_build_controller.AlarmAndWarningSave(argu).then(function (res) {
    ...
    computed: {
        ...
        disabled: function () {
            ...
            var bool = ((+_that.AlarmAndWarning[0].alarm.switch && _that.AlarmAndWarning[0].alarm.sensitive.SELECTED == null) || (+_that.AlarmAndWarning[0].warning.switch && _that.AlarmAndWarning[0].warning.sensitive.SELECTED == null)) ? true : false;
        ...
        BuildingEnergyOutlinesCed: function () {
            ...
            return _that.BuildingEnergyOutlines.map(function (item) {
                ...
                // 报警预警百分比转换
                item.alarmRatio = item.alarmRatio ? item.alarmRatio.toFixed(1) + '%' : '';
                item.warningRatio = item.warningRatio ? item.warningRatio.toFixed(1) + '%' : '';

                // 计算出百分比
                item.proportion = item.monthBudgetData ? (((item.monthEnergyData / item.monthBudgetData) || 0).toFixed(5) || 0) : 0;
                ...
                // 百分比数字
                item.proportionNumber = (+item.proportion > 0.01) ? +_that.convertPercentage(+item.proportion).toFixed(1) : (+item.proportion != 0) ? +_that.convertPercentage(+item.proportion).toFixed(3) : 0;

                item.proportionNumber = item.proportionNumber == Infinity ? 100 : item.proportionNumber;
                // 百分比显示的文字
                item.proportionName = item.proportionNumber == Infinity ? '0' : item.proportionNumber + '%';

                item.proportionNumberBai = item.proportionNumber > 100 ? 100 : item.proportionNumber;
    ...
    watch: {
        alarmwindow: function (bool) {
            ...
            if (bool) {
                more_build_controller.FNPJ_AlarmAndWarningQuery({
        ...
        onBlock: function (newValue) {
            ...
            if (newValue == 'detail') {
                setTimeout(function () {
                    canNotSelectFutureDay(+new Date($("#daterightdate").psel().startTime), 'daterightdate');
                }, 0);
            }
        }
    },
    beforeMount: function () {
        ...
        setTimeout(function () {
            ...
            canNotSelectFutureDay($("#divCalendar").psel().startTime, "divCalendar", true);
        }, 0);
    }
})